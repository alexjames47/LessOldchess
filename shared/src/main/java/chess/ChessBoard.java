...
public class ChessBoard {
    ...
    public ChessBoard(ChessBoard newBoard){
        for(int i=0; i<8; i++){
            for(int j=0; j<8; j++){
                if(newBoard.getCurrentBoard()[i][j] != null){
                    this.currentBoard[i][j] = new ChessPiece(newBoard.getOccupiedSpaceColor(new ChessPosition(i+1,j+1)),newBoard.getCurrentBoard()[i][j].getPieceType());
                }
            }
        }
    }
